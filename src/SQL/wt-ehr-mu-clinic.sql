...
set dx_tbl = $db_flag || '.' || $ehr_schema || '.LDS_DIAGNOSIS';
set px_tbl = $db_flag || '.' || $ehr_schema || '.LDS_PROCEDURES';
...
select * from identifier($dx_tbl) limit 5;
select * from identifier($px_tbl) limit 5;
